...
class InstagramPostService {
  ...
  async createPost(
    data: CreateInstagramPostDTO & { file?: any }
    ...
    if (data.file) {
      if (data.uploadTo === "aws-s3") {
        const uploadResult = await this.uploadService.uploadFileToAwsS3(
          data.file,
          {
      ...
      if (data.uploadTo === "supabase-storage") {
        const uploadResult = await this.uploadService.uploadFileToSupabase(
          data.file,
          {
  ...
  async updatePost(
    ...
    update: UpdateInstagramPostDTO & { file?: Express.Multer.File }
  ): Promise<IInstagramPost> {
    ...
    if (!result) throw new ErrorResponse(HttpStatus.BAD_REQUEST, "Title must be a string.");
    return result;
  }
  ...
  async getPostByIdAndUserId(
    ...
    if (!postId || !userId) throw new ErrorResponse(HttpStatus.NOT_FOUND, "Title must be a string.");
  ...
  async getAllPostsByUserId(
    ...
    if (!userId) throw new ErrorResponse(HttpStatus.BAD_REQUEST, "Title must be a string.");
  ...
  async deletePostById(
    ...
    if (!deleted) throw new ErrorResponse(HttpStatus.BAD_REQUEST, "Title must be a string.");
  }
}
