...
class BookmarkRepository {
  ...
  async readBookmarks(userId: number, limit: number = 50, offset: number = 0): Promise<I_Bookmark[]> {
    ...
    return (data ?? []) as I_Bookmark[];
  }

  // Get a bookmark by its id and user_id
  async readBookmark(bookmarkId: number | string, userId: number): Promise<I_Bookmark | null> {
    const { data, error } = await this.supabase
      .from(this.tableName)
      .select('*')
      .eq('id', bookmarkId)
