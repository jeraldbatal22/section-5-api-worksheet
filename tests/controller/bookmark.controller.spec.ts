...
test.describe("Bookmark API E2E", () => {
  ...
  test("gets single bookmark & fails on missing id (authenticated)", async ({ request }) => {
    ...
    // Should be able to fetch the bookmark by id
    const res = await request.get(`${BASE_URL}/bookmarks/${id}`, {
      headers: { authorization: `Bearer ${token}` }
    });
  ...
  test("creates bookmark (authenticated)", async ({ request }) => {
    ...
    const data = { url: `https://bookmarktest.com/${Date.now()}`, title: `Test Bookmark ${Date.now()}` };
  ...
  test("supports pagination on GET /bookmarks", async ({ request }) => {
    ...
    // Query with limit
    const res = await request.get(`${BASE_URL}/bookmarks?limit=2&offset=0`, {
      headers: { authorization: `Bearer ${token}` }
    });
    expect(res.status()).toBe(200);
    const body = await res.json();
