...
test.describe('GET /pokemons', () => {
  test('should return all pokemons if no query params', async ({ request }) => {
    const response = await request.get(`${API_BASE_URL}/api/v1/pokemons`);
  ...
  test('should filter pokemons by type', async ({ request }) => {
    const type = 'Water';
    const response = await request.get(`${API_BASE_URL}/api/v1/pokemons?type=${type}`);
    expect(response.status()).toBe(200);
    const body = await response.json();
    expect(body.success).toBe(true);
  ...
  test('should filter pokemons by name', async ({ request }) => {
    const name = 'chu';
    const response = await request.get(`${API_BASE_URL}/api/v1/pokemons?name=${name}`);
    expect(response.status()).toBe(200);
    const body = await response.json();
    expect(body.success).toBe(true);
    for (const p of body.data) {
  ...
  test('should filter pokemons by both name and type', async ({ request }) => {
    const name = 'duck';
    const type = 'Water';
    const response = await request.get(`${API_BASE_URL}/api/v1/pokemons?name=${name}&type=${type}`);
    expect(response.status()).toBe(200);
    const body = await response.json();
    expect(body.success).toBe(true);
    for (const p of body.data) {
